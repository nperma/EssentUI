...
- added `scorehud` property in `config.js`
- Added `veinminer` property in `config.js` (default: `false`) 
- Added command `scorehud` to toggle scorehud visibility
- added KickUI
- added `punishment` property in `config.js`
- added BanUI
- added SuspendUI
- added UnbanUI
- added UnsuspendUI (featured only for owner)

#### 🛠️ Changes & Fixes  

**Scripts**
- rebuilt quick-db to make optimize & fast storage data

#### ❌ Removed  

**Scripts**  
- Removed LoggerSystem cause have a lot bugs
- Removed `config.logger` useless option# Changelog Page  
**Language**: English (International)  
...
**Scripts**  
- Removed LoggerSystem cause have a lot bugs
- Removed `config.logger` useless option
